...
export function StudentVersionsPage() {
  ...
  if (!me.data.project_id) return <AppShell active="提交记录" role="student" campSlug={me.data.camp.slug} avatar={me.data.user.display_name}><main className="dashboard-content narrow-content"><p className="breadcrumb">{me.data.camp.name}　/　提交记录</p><h1>还没有可以查看的提交</h1><p className="empty-copy">老师为你创建作品后，每次提交都会按时间留在这里。</p></main></AppShell>;
  ...
  return <AppShell active="提交记录" role="student" campSlug={me.data.camp.slug} avatar={me.data.user.display_name}>
    <main className="dashboard-content version-history-page">
      <header className="page-heading"><div><p className="breadcrumb">{me.data.camp.name}　/　提交记录</p><h1>每一次提交，都有回音</h1><p className="review-counts"><b>{versions.data.items.length}</b> 条最近记录　·　按最新提交排序</p></div><ModeTabs campSlug={me.data.camp.slug} active="app" /></header>
      {versions.data.items.length ? <section className="panel version-history-panel"><header><div><p className="eyebrow">版本时间线</p><h2>提交与审核记录</h2></div><span>诊断完成度会在检查结束后显示</span></header><ol className="version-history-list">{versions.data.items.map((version) => <VersionHistoryRow key={version.id} version={version} />)}</ol></section> : <section className="panel version-history-empty"><span>◔</span><strong>还没有提交过版本</strong><p>完成第一版作品后提交，老师的审核结果和诊断完成度都会记录在这里。</p></section>}
    </main>
  </AppShell>;
}
