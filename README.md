...
> 状态：核心黄金路径、学员网页直传、VibeHub Deploy 一次式安装与部署、学生自助创建多个作品、多营地连接和老师端学员说明均已实现。
...
- 推荐使用 Codex、Claude Code、WorkBuddy 或兼容 Agent：打开 `/install`，只需复制一次页面上的完整自然语言给 AI。AI 会识别系统与 Agent，按清单逐项核对文件字节数和 SHA-256 后安装或更新 VibeHub Deploy，在需要时询问个人邀请码，先检查当前目录的作品绑定，再创建或关联目标作品并立即提交。学员不复制、不执行终端命令。
- 同一学生可以在一个营地创建任意多个独立作品，不需要老师再发邀请码。AI 对新目录执行 `project create`，对已存在的作品执行 `project link`；每个目录固定到一个作品连接，提交不会跟随全局当前连接而串项目。
- 已有 HTML、ZIP 或网页文件夹也可以打开 `/login`，用老师发放的个人邀请码登录，再从“提交我的游戏”切换到网页上传备用方式。
- VibeHub Deploy 的技术标识和安装目录名是 `vibehub-deploy`，展示名是 **VibeHub Deploy**。官网仍通过旧公开路径 `/downloads/vibehub-skill/` 发布清单与白名单文件，以保持已发出链接兼容；当前多作品分发版本必须精确为 `1.0.1`，在线安装器会核对大小与 SHA-256，失败时不替换已有 Skill。
- 老师登录管理端的邀请码页面后，可复制一段 AI 推荐、网页备用的完整说明；生成学员邀请码后，还可复制只包含该学员个人邀请码的一段完整说明。老师角色邀请码的生成结果不会生成绑定该码的学员转发文案。
