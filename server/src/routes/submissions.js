...
export default async function submissionRoutes(app, { diagnosisQueue }) {
  app.post('/api/projects/:id/versions', { preHandler: authRequired(['student']) }, async (req, reply) => {
    ...
    const permit = browserSubmissionGuard.acquire(project.id);
    if (!permit.ok) {
    ...
    try {
      for await (const part of req.parts()) {
        if (part.type === 'file' && part.fieldname === 'bundle') {
          ...
          tmpFile = join(paths.tmp, `web_${nanoid(10)}.upload`);
      ...
      if (!tmpFile) return sendError(reply, 'missing_bundle', 400, '没有收到上传的内容包。');
      ...
      req.log.info({ project_id: project.id, format: extname(filename || '').toLowerCase() || 'unknown', bundle_size: statSync(tmpFile).size }, '网页提交已接收');
