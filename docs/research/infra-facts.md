...
已拍板的作品地址是主域路径式：
`https://supermind-ai.cn/vibehub/<username>/<projectname>/`。安全复核后，版本预览设计改为
`https://<pid16>.preview.supermind-ai.cn/vibehub/_preview/<pid16>/`，让每个待审版本拥有独立 origin。控制台与 API 使用 `https://hub.supermind-ai.cn`。**截至本文记录，预览泛解析和通配证书尚未做生产实测，属于待验证上线前置条件，不是已存在的基础设施事实。**
...
| 申请正式域与控制台普通证书 | 为 `supermind-ai.cn`（可含 `www`）和 `hub.supermind-ai.cn` 配置普通证书，可走 **HTTP-01** | 443 开通后执行 certbot，80 端口已通 |
| 配置 `hub.supermind-ai.cn` A 记录 | 指向 `<南京机 IP，见 server-vault TC_NANJING_*>` | 需确认该机归属哪个腾讯云账号后在 DNS 控制台操作 |
| 配置预览泛解析与通配证书 | `*.preview.supermind-ai.cn` 指向同一服务器，并签发匹配该域的通配证书（通常需 DNS-01） | **尚未实测**；DNS、证书和 Nginx 三项完成后才能启用独立预览 origin |

> 这些事项是上线前置项。**在 443 打通前，作品网址只能是 http，不影响功能但影响观感与部分浏览器 API（如地理位置、麦克风录音需要 HTTPS）**——注意原型里的「城市声音地图」要录音，**这意味着 HTTPS 不是可选项而是功能前提**。
...
VibeHub 的部署管道可以复用这套目录约定与权限机制，但需要主域路径式正式作品路由、`hub.supermind-ai.cn` 控制台/API server 块，以及一个匹配 `*.preview.supermind-ai.cn` 的正则 server 块。泛解析避免逐个 preview 配置 DNS 或 server 块。
