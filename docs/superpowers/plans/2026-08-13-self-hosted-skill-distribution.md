...
Merge the isolated branch into main without overwriting unrelated Shanghai-camp changes. Push main. Build the console with `VITE_API_BASE` and `VITE_PUBLIC_APP_URL` only; upload to `/var/www/vibehub-console/releases/<release>` and atomically switch `/var/www/vibehub-console/current`, with nginx root pointing at that link. Keep the previous static release for rollback. The server does not require a new release unless review changes server runtime code.
