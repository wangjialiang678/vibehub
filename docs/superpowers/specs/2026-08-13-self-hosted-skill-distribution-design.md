...
在线引导器由安装页下载到本机临时目录，因此安装命令同时把当前官方分发根地址作为 `--base-url` 传入。生产只接受无用户名、密码和片段的 HTTPS 地址；本地自动化测试额外允许 loopback HTTP。引导器只从这个地址读取清单与白名单文件，不执行清单以外的内容，不读取邀请码，也不接触 VibeHub 登录凭证。
...
老师登录管理端的邀请码页面后，始终能看到“发给学员的使用说明”，不必等到新建邀请码后才知道如何组织通知。说明明确并列两条路径：

1. **网页登录**：打开 `/login`，输入个人邀请码，然后在网页里直接上传 HTML、ZIP 或网页文件夹。
2. **AI 部署**：打开 `/install`，按 macOS 或 Windows 安装部署 Skill，回到 WorkBuddy、Codex 或其他兼容 Agent，先说“使用邀请码加入 VibeHub”，作品完成后说“部署我的游戏”。

固定说明使用 `CAMP-XXXX` 占位符，方便老师提前了解和复制模板；每次生成学员邀请码后，页面继续为每个明码生成独立的完整说明，自动替换为该学员自己的邀请码。页面不持久化、不记录明码，老师邀请码也不生成学员说明。营地名和公开地址来自当前会话与构建配置，不写死深圳或任何城市，因此后续其他营地直接复用。
...
- 在老师邀请码管理页增加持久可见的双路径说明，并让每个新生成的学员邀请码可复制对应完整通知。
...
7. **老师端**：无邀请码时仍显示网页登录与 AI 部署模板；新建学员邀请码后，每段说明只含对应明码、营地名、`/login` 与 `/install`；老师邀请码不显示学员通知。
...
- 老师在管理端能随时查看两种学员路径，并可在生成邀请码时一键复制每位学员的完整说明。
